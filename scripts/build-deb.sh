...
if dpkg-deb --fsys-tarfile "$package_temporary" | tar -tf - \
  | grep -Eq '(^|/)(voice\.json|vocabulary\.json|corrections\.json|adaptive-corrections\.json|data-collection\.json|interaction\.json|microphone-priority\.json|output-style\.json|output-target\.json)$'; then
