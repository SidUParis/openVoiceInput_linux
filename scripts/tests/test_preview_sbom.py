...
def write_test_bundle(root: Path, *, project_module_content: bytes = b"") -> None:
    ...
    wheelhouse.mkdir()
    project_dist_info = "murmur_ime_voice-0.1.0a5.dist-info"
    write_test_wheel(
        wheelhouse,
        filename="murmur_ime_voice-0.1.0a5-py3-none-any.whl",
        name="murmur-ime-voice",
        version="0.1.0a5",
    ...
    def test_deterministic_cyclonedx_inventory_and_dependency_graph(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                ...
                hashlib.sha256(
                    (
                        root / "wheelhouse/murmur_ime_voice-0.1.0a5-py3-none-any.whl"
                    ).read_bytes()
                ).hexdigest(),
            )
            self.assertEqual(
            ...
            self.assertEqual(
                graph["pkg:pypi/murmur-ime-voice@0.1.0a5"],
                [
