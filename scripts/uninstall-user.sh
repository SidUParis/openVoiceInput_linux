...
vocabulary, correction, and data-collection settings are retained. Any dataset
in a user-selected external directory is never removed.
EOF
}
...
printf '%s\n' \
  "$removed_summary" \
  "The private API-key, vocabulary, correction, and data-collection settings were retained under the XDG config directory." \
