...
从 [v0.1.0-alpha.5 Release](https://github.com/SidUParis/openVoiceInput_linux/releases/tag/v0.1.0-alpha.5)
下载 `.deb`，然后在下载目录运行：
...
冲突或跳过”，也允许在应用无法提供可信 surrounding text 时显式提交“识别原文 /
修改后整句”。例如把 `bench mark` 修成 `benchmark` 时，只形成这个短语的规则，
不会把整句一起记住。
