...
4. 正常开始和结束听写；
5. 在 1.6.1+ 兼容控制器的悬浮按钮／托盘中看到“上一条终稿已复制；剪贴板可能
   已被其他应用覆盖”后，先确认远端目标输入框，再由你按 `Ctrl+V`。未运行该
   控制器时，可用已安装的 `murmur-voice-daemon status` 查看
   `clipboard-ready`；设置窗口的状态不是实时录音浮层，需要重新打开后才刷新。
   这个状态只证明写入当时成功，不承诺当前剪贴板仍未被其他应用改写。
