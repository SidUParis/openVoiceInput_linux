...
- 每次听写前按用户保存的完整顺序重新选择输入；推荐默认顺序为“大疆 >
  耳麦 > 其他外接 > 电脑内置”，也可在设置窗口上下调整。不可用或同类歧义
  会继续尝试下一类；大疆仍会检查发射器在线状态；
...
设置窗口保存四类麦克风的完整优先顺序：大疆、耳麦、其他外接和电脑内置。
推荐默认是“大疆 > 耳麦 > 其他外接 > 电脑内置”，但用户可以调整。每次新
听写都会重新读取顺序和枚举 source；每类内部依次使用明确保存的设备、该类
当前系统默认、或唯一候选，同类仍有歧义就继续下一类而不是猜测。耳麦断开时
可自动降级到下一类，重新连接后下一次听写会再次按顺序选择。

大疆 Mic Mini 2 的 USB 接收器在发射器关机后仍会注册一个输入 source，所以
守护进程仍做有界 link-state 检查。能证明在线时，大疆在用户保存的位置成为
可用候选；能证明离线时排除静音接收器；状态未知时不会把它提升到已知可用的
输入之前。仅当它已经是系统默认、又没有任何可选非大疆或可恢复输入时，才可
作为最后连续性回退。只有已经提供输入 source 的 HSP/HFP 蓝牙通话模式才算
耳麦麦克风；程序不会主动把 A2DP 播放切到通话模式。选择只作用于新建的应用
录音流，不改变播放 sink，也不请求修改系统默认 source。一次听写开始后不会
中途换麦；下一次听写才重新判断。
...
管理的文件。出于防止误删凭据或珍贵数据的考虑，私有 Key、词表、纠错、
`microphone-priority.json` 和 `data-collection.json` 会保留；用户所选目录中的
