...
会隔离。设置页显示最近结果与原因，也提供“识别原文 / 修改后整句”的显式
fallback，供无法暴露可信 IBus surrounding text 的应用使用。
...
若要保留候选数据，在设置页勾选 **在所选目录保留 WAV 与未经复核的
provider_final**，选择一个已经存在的绝对目录，再点击 **保存数据留存设置**。
目录可以
