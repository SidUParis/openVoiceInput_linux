...
- 当前 ASR：用户选择在线服务并自备对应 Key；音频只在主动听写后发送给所选
  服务。火山引擎默认且唯一用真实 Key 验收，Qwen/OpenAI 仍是实验后端
...
Ubuntu 24.04 和用户自己选择的在线 ASR 账户；本地 ASR 与更广泛发行版支持仍在
路线图中。
...
2. **从明确修改中学习**：只在同一焦点、有限时间和严格单一替换成立时生成
   个人纠错，不做全局键盘监听。
3. **数据由用户决定**：数据采集默认关闭；启用后，WAV、未审核的供应商原文
   和机器生成的实际交付结果只写到用户选择的本地或已挂载目录。
...
- 24–34 秒：用户自定义麦克风优先级，以及首选设备断开后的下一句动态回退；
- 34–45 秒：默认关闭的数据采集以及 WAV/JSON 的字段角色；
- 45–55 秒：当前 Ubuntu/在线服务边界、GPLv3、GitHub 地址和本地 ASR 路线图。
...
- “不会把音频发出电脑”；当前在线识别路径会在主动听写后把音频发送给用户
  所选服务，且计费、地域处理与服务端留存遵循该服务和账户配置；
